...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initCharts() {
    ...
    d3.selectAll("[data-chart='scalebar']").each(function () {
      drawScalebar(this);
    });
  }

  /* ================================================================
     SCALEBAR — barras horizontais em escala real 0–100, com eixo,
     rótulos abaixo do eixo e contagem animada até o valor
     ================================================================ */
  function drawScalebar(el) {
    var max = parseFloat(el.dataset.max) || 100;
    var rows = Array.prototype.slice.call(el.querySelectorAll(".scalebar__row"));
    if (!rows.length) return;

    var data = rows.map(function (r) {
      return {
        label: r.dataset.label || "",
        value: parseFloat(r.dataset.value) || 0,
        color: r.dataset.color || "var(--color-ink)"
      };
    });
    ...
    var MR = 8;
    var BAR_H = 40;
    var H = TOP + ROW_H * data.length + 10;

    var x = d3.scaleLinear().domain([0, max]).range([ML, W - MR]);
    ...
    var svg = d3
      ...
      .attr(
        ...
        data
          .map(function (d) {
            return fmt(d.value, 0) + "% " + d.label;
          })
          .join("; ") + "."
      );

    var dur = prefersReducedMotion ? 0 : 1200;

    data.forEach(function (d, i) {
      var oy = TOP + i * ROW_H;
      var target = x(d.value);
      var axisY = oy + BAR_H + 52;
      var g = svg.append("g");

      // Grade vertical da escala (percepção da proporção real).
      ...
      // Valor (conta de 0 até o alvo, junto com a barra).
      var val = g
        .append("text")
        .attr("x", x(0))
        .attr("y", oy + 20)
      ...
      // Eixo inferior com ticks.
      var axisG = g
        .append("g")
        .attr("transform", "translate(0," + axisY + ")")
        .call(
          d3.axisBottom(x).tickValues([0, 25, 50, 75, 100]).tickFormat(function (t) {
            return t + "%";
          })
        );
      axisG
      ...
      axisG.select(".domain").style("stroke", "var(--color-ink)");

      // Rótulo da categoria, abaixo do eixo.
      var cat = g
        .append("text")
        .attr("x", x(0))
        .attr("y", axisY + 28)
      ...
      onEnter(el, function () {
        fill
          ...
          .duration(dur)
          .ease(d3.easeLinear)
          .attr("width", Math.max(0, target - x(0)));
        val
          .transition()
          .delay(i * 200)
          .duration(dur)
          ...
          .attr("x", target)
          .tween("text", function () {
            var it = d3.interpolateNumber(0, d.value);
            return function (t) {
              this.textContent = fmt(it(t), 0) + "%";
            };
          });
        cat.transition().delay(i * 200).duration(dur).style("opacity", 1);
      });
    });
  }
